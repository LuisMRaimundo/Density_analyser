...
def calcular_densidade(notas: List[float], 
    ...
    # Pré-normalizar e validar todas as notas de uma só vez
    notas_normalizadas = [normalize_note_string(n) for n in notas]
    notas_validas = [n for n in notas_normalizadas if is_valid_note(n)]

    if usar_ponderacao_perceptual:
        massa_ponderada = 0.0
        for nota in notas_validas:
            massa_ponderada += get_register_weight(nota)
...
def calcular_densidade_intervalar(notas: List[float]) -> float:
    """
    Calcula a densidade de intervalo de forma eficiente.
    Usa apenas intervalos adjacentes e o intervalo total.
    
    Args:
        notas: Lista de valores MIDI
        
    Returns:
        float: Densidade intervalar média
    """
    if len(notas) < 2:
        return 0.0
    ...
    # Intervalos adjacentes (mais relevantes perceptualmente)
    intervalos_adjacentes = [
        notas_ordenadas[i+1] - notas_ordenadas[i] 
        for i in range(len(notas_ordenadas)-1)
    ]
    
    # Adicionar o intervalo total (envelope)
    intervalo_total = notas_ordenadas[-1] - notas_ordenadas[0]
    
    # Média ponderada: adjacentes têm mais peso
    if intervalos_adjacentes:
        media_adjacentes = np.mean(intervalos_adjacentes)
        # 70% peso para adjacentes, 30% para o total
        densidade_intervalar = 0.7 * media_adjacentes + 0.3 * intervalo_total
    else:
        densidade_intervalar = intervalo_total
...
def analisar_densidade_completa(notas: List[float], 
                               pesos: List[float] = None) -> Dict[str, float]:
    """
    Análise completa de densidade com múltiplas métricas.
    ...
    resultado = {
        'densidade_basica': calcular_densidade(notas),
        #'densidade_intervalar': calcular_densidade_intervalar(notas, gui.get_perceptual_weighting_status()),
        'densidade_intervalar': calcular_densidade_intervalar(notas, usar_ponderacao_perceptual),
    ...
    # Adicionar densidades por registro
    densidades_registro = calcular_densidade_por_registro(notas)
    resultado.update({
        f'densidade_{k}': v for k, v in densidades_registro.items()
    })
